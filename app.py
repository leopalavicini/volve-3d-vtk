...
for points in surveys:
    points_dict["{}".format(points)] = np.array(list(zip(surveys[points].inline,surveys[points].xline,surveys[points].TVD)))
...
reservoir_hor = pd.read_csv(r'data/Horizons/Hugin-reservoir.dat', names=['x','y','inline','xline','z'], sep='\t')
...
grid_points = np.array([100300, 21450, 0, 100300, 25360, 0, 102600, 21450, 0, 102600, 25360, 0, 100300, 21450, 3750, 100300, 25360, 3750, 102600, 21450, 3750, 102600, 25360, 3750])
...
controls = [
    ...
    dbc.FormGroup(
        [
            ...
            dbc.Checklist(
                options=[{"label": "Inline", "value": "inline"}, {"label": "Xline", "value": "xline"}, {'label':'Depth Slice','value':'z'}],
...
app.layout = dbc.Container(
    fluid=True,
    children=[
    html.Div([html.H3("Geoscience Data Visualizer",
            style={'fontSize': '2vw', 'lineHeight': 1.3, 'letterSpacing': '-1px', 'marginBottom': '0px',
                   'textAlign': 'center', 'marginTop': '0px',  'fontFamily': "sans-serif"}
            ),
            html.H5("Volve Dataset",
            style={'fontSize': '1.5vw', 'lineHeight': 1.5, 'letterSpacing': '-0.5px', 'marginBottom': '0px',
                   'textAlign': 'center', 'marginTop': '0px', 'fontFamily': "sans-serif"}
            ),
            dcc.Markdown("Volve is a Norwegian Oil Field in the North Sea, that in 2018, Equinor and the Volve license partners decided to releasea a big dataset containing all kinds of data, from seismic to production related to the field, all of these under an Open License, that you can check [here.](https://www.equinor.com/content/dam/statoil/documents/what-we-do/Equinor-HRS-Terms-and-conditions-for-licence-to-data-Volve.pdf)",
            style={'fontSize':'.7vw', 'marginBottom': '1vh',
                   'textAlign': 'center', 'marginTop': '0px', 'fontFamily': "sans-serif"}
            )]
    ),
    html.Div([
                ...
                dbc.Row([
                    ...
                    style={'width':'15vw'}), style={'margin-left':'5vw'}, width={'size':2}),
...
def inl_value(in_val, xl_val, z_val):
    ...
    return f'Inlines: {in_display}', f'Xlines: {xl_display}', f'Depth slice: {z_display} m'
...
def update_slice_property(i, j, k, well, hor, seis):
    ...
    return render_call, i, j, k, act, act2, act3, act4, act5, act6, act7, act8, act9, act10, act11, act12
